    ...
    $(window).scroll(function(){
        if ($(document).height() - $(this).scrollTop() - $(this).height()<10 && isGetFlag){
            ...
            $.ajax({
                ...
                data : data,
                beforeSend:function(){
                    $(insertBox).append('<div id="get_more_loading" class="get_more_loading"><img src="../addons/yc_youliao/public/images/loading.gif"> 正在加载</div>');
                },
                success: function(data){
                    if(data.status == 'ok'){
...
common.createWebUrl = function(dostr,opstr,obj){
    var str = '&do='+dostr+'&op='+opstr;
    for(t in obj){
        str += '&'+t+'='+obj[t];
    }
