...
function pc_init(){
	//设置活动名称
	$('input[name="model[title]"]').val('五一劳动节安踏新品送不停 转动大盘赢豪礼');
	//设置活动封面图片
    var picurl_ipt =$('input[name="model[pic_url]"]');
    picurl_ipt.val(module_url + "resource/template/tmp/tmp66/cover.jpg");
    picurl_ipt.parent().next().children().first().attr("src", module_url + "resource/template/tmp/tmp66/cover.jpg");
	//设置活动内容
    var uecontent = UE.getEditor('model[content]');
    uecontent.ready(function() {//编辑器初始化完成再赋值
        uecontent.setContent('<p style="white-space: normal;"><strong><span style="color: rgb(255, 0, 0); font-size: 20px;">本次活动为系统活动仅供参考！切勿当真！</span></strong></p><p style="white-space: normal;"><span style="color: rgb(255, 0, 0);"><strong>=======================</strong></span>&nbsp;</p><p>安踏2020春季新款运动鞋送好礼啦，参与活动即可抽取您的专属免单。</p><p style="white-space: normal;"><br/></p><p><span style="font-size: 18px;">***免单转出来***</span></p><p>参加活动：点击转盘上的“点击抽奖”即可参加活动。</p><p>好礼一起拿：朋友参加自己可以获得额外一次机会。<br/>凭参与记录进店就有惊喜送。</p><p style="white-space: normal;"><br/></p><p style="white-space: normal;"><span style="font-size: 18px;">***新品展示***</span></p><p style="white-space: normal;"><span style="font-size: 18px;"></span></p><p><img src='+module_url+'resource/template/tmp/tmp66/content01.jpg width="100%" style=""/></p><p><img src='+module_url+'resource/template/tmp/tmp66/content02.jpg width="100%" style=""/></p><p><img src='+module_url+'resource/template/tmp/tmp66/content03.jpg width="100%" style=""/></p><p><img src='+module_url+'resource/template/tmp/tmp66/content04.jpg width="100%" style=""/></p><p style="white-space: normal;"><span style="font-size: 18px;"><br/></span><br/></p>');
    });
	//设置活动分享标题
    $('input[name="model[share_title]"]').val('{NICKNAME}安踏送出免单大奖，好运惊喜转出来');
	//设置活动分享图标
    var share_thumb_ipt =$('input[name="model[share_thumb]"]');
    share_thumb_ipt.val(module_url + "resource/template/tmp/tmp66/cover.jpg");
    share_thumb_ipt.parent().next().children().first().attr("src", module_url + "resource/template/tmp/tmp66/cover.jpg");
	//设置活动分享描述
...
function mobile_init(){
    ...
    $('input[name="model[title]"]').val('五一劳动节安踏新品送不停 转动大盘赢豪礼');
	//设置活动封面图片
    $("#pic_url").attr("value", module_url + "resource/template/tmp/tmp66/cover.jpg");
    $(".js-image-pic_url").css({"position":"absolute" ,"opacity":"0"});
    $(".js-image-pic_url").next("div").html("<img src="+module_url+"resource/template/tmp/tmp66/cover.jpg />");
    //设置活动内容
	artEditor.clear();
    ...
    $('input[name="model[share_title]"]').val('{NICKNAME}安踏送出免单大奖，好运惊喜转出来');
	//设置活动分享图标
    $("#share_thumb").attr("value", module_url + "resource/template/tmp/tmp66/cover.jpg");
    $(".js-image-share_thumb").css({"position":"absolute" ,"opacity":"0"});
    $(".js-image-share_thumb").next("div").html("<img src="+module_url+"resource/template/tmp/tmp66/cover.jpg />");
	//设置活动分享描述
    $('textarea[name="model[share_desc]"]').val('{NICKNAME}安踏送出免单大奖，好运惊喜转出来');
	//设置活动门店地址
    $('input[name="model[store_map_list][0][name]').val('王府井');
